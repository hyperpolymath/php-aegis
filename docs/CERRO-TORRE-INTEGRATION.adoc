...
This guide explains how to deploy php-aegis WordPress using the
*Verified Container Ecosystem*: Cerro Torre (builder), Vörðr (runtime),
and Svalinn (gateway).
...
|*Formal Verification* |Ada/SPARK verified tooling |No formal
verification
...
|*Security* |SELinux enforcing, threshold signing |Optional security
features
...
Key sections: - *[package]*: Package metadata - *[dependencies]*: PHP
8.2, WordPress 6.4, php-aegis 0.2 - *[build]*: Declarative build steps -
*[runtime]*: Container runtime configuration - *[security]*: SELinux
policy, capabilities, network isolation - *[attestations]*: SBOM,
provenance, threshold signing
...
SELinux policy (auto-generated by Cerro Torre): - *Read-only WordPress
files*: PHP can read but not write WordPress core - *Writable uploads*:
PHP can write to uploads directory only - *Rate limit storage*: PHP can
write rate limit data - *Network access*: PHP can connect to MySQL only

==== 2. Capability Dropping

Container drops all capabilities except: - `+CHOWN+` - Change file
ownership (for wp-content) - `+SETGID+` - Set group ID (for www-data) -
`+SETUID+` - Set user ID (for www-data) - `+NET_BIND_SERVICE+` - Bind to
port 9000
...
`+/var/www/html/wp-content/cache+` (cache) - `+/var/ratelimit+` (rate
limiting data) - `+/tmp+` (temporary files)

==== 4. Network Isolation

*Egress (outbound)*: - WordPress API (api.wordpress.org:443) -
Composer/Packagist (packagist.org:443) - IndieWeb endpoints
(*.indieweb.org:443)

*Ingress (inbound)*: - Port 9000 (PHP-FPM) from nginx container only

==== 5. Threshold Signing

Container requires 2-of-3 signatures: - maintainer-1 - maintainer-2 -
release-bot
...
Build attestations logged to: - Cerro Torre transparency log - Sigstore
Rekor
...
*Trade-off*: Slightly slower for significantly higher security
guarantees.
...
`+docker-compose up+` → `+svalinn-compose up+` - `+docker-compose ps+` →
`+svalinn-compose ps+` - `+docker-compose logs+` →
`+svalinn-compose logs+`
...
[arabic]
. *Read Cerro Torre docs*: https://github.com/hyperpolymath/cerro-torre
. *Read Vörðr docs*: https://github.com/hyperpolymath/vordr
. *Read Svalinn docs*: https://github.com/hyperpolymath/svalinn
. *Join community*:
https://github.com/hyperpolymath/verified-container-spec

=== References

* https://github.com/hyperpolymath/verified-container-spec[Verified
Container Spec]
* https://github.com/hyperpolymath/cerro-torre/blob/main/spec/manifest-format.md[Cerro
Torre Manifest Format]
* https://in-toto.io/[in-toto Attestation]
* https://slsa.dev/provenance/[SLSA Provenance]
* https://spdx.dev/[SPDX SBOM]
